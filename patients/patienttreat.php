      ...
      <div class="table-responsive">
        <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            ...
            $check=$treatments->getpatienttreat();
             $today = date("Y-m-d"); 
            ...
            <tr>
              ...
              <td>
                 <?php echo "<a href='appointmentsreport.php?id=" .$row['Id']. "' class='btn btn-info'>Report</a>";?>
                <a href="#" class="start_chat btn btn-primary"
                    data-treatid="<?php echo $row['Id'];?>"
                    data-patientname="<?php echo $row['Patientname'];?>"
                    data-doctorname="<?php echo $row['Doctorname'];?>"
...
<!-- <script>
 ...
 <div class="modal fade" id="querymodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   ...
   <script>
    ...
    function make_chat_dialog_box(to_treat_id, to_doctor_name)
     {
      var modal_content = '<div id="user_dialog_'+to_treat_id+'" class="user_dialog" title="You have chat against Treatment '+to_treat_id+'">';
      modal_content += '<div style="height:340px; border:1px solid #185479;background-color:white; overflow-y: auto; margin-bottom:24px; padding:16px;" class="chat_history" data-treatid="'+to_treat_id+'" id="chat_history_'+to_treat_id+'">';
      modal_content += fetch_patient_chat_history(to_treat_id);
      modal_content += '</div>';
      modal_content += '<div class="form-group">';
      modal_content += '<textarea name="ask_query'+to_treat_id+'" id="ask_query'+to_treat_id+'" class="form-control" style="border-radius:0;margin-top:-25px;" Placeholder="Please type your message"></textarea>';
     ...
     $(document).on('click', '.start_chat', function(){
      ...
      var to_doctor_name = $(this).data('doctorname');
      make_chat_dialog_box(to_treat_id, to_doctor_name);
...
$(document).on('click', '.send_chat', function(){
  ...
  $.ajax({
   url:"../pages/insert_patient_chat.php",
   method:"POST",
...
function fetch_patient_chat_history(to_treat_id)
 {
  $.ajax({
   url:"../pages/fetch_patient_chat_history.php",
   method:"POST",
 ...
 function update_chat_history_data()
 {
  $('.chat_history').each(function(){
   var to_treat_id = $(this).data('treatid');
   fetch_patient_chat_history(to_treat_id);
  });
 }
     ...
     <?php
function count_unseen_message($from_patient_id, $to_treat_id, $db)
{
  $db = new PDO("mysql:host=localhost;dbname=dentalclinic",'root','');
 $query = "
 SELECT * FROM queries 
 WHERE from_patient_id = '$from_patient_id' 
 AND to_treat_id = '$to_treat_id' 
 AND status = '1'
 ";
